...
final readonly class BattleStarPlayerDTO
{
    /**
     * @param string                                                  $tag
     * @param string                                                  $name
    ...
    public static function fromArray(array $data): self
    {
        if (!(isset($data['tag']) && is_string($data['tag']) && !empty(trim($data['tag'])))) {
            throw InvalidDTOException::fromMessage(
                "Invalid or missing 'tag' field in battle star player's data."
            );
        }

        if (!(isset($data['name']) && is_string($data['name']) && !empty(trim($data['name'])))) {
            throw InvalidDTOException::fromMessage(
                "Invalid or missing 'name' field in battle star player's data."
            );
        }

        if (!(isset($data['brawler']) && is_array($data['brawler']))) {
            throw InvalidDTOException::fromMessage(
                "Invalid or missing 'brawler' field in battle star player's data."
            );
        }
        ...
        if (!(isset($brawlerData['id']) && is_numeric($brawlerData['id']))) {
            throw InvalidDTOException::fromMessage(
                "Invalid or missing 'id' field in battle star player brawler's data."
            );
        }

        if (!(isset($brawlerData['name']) && is_string($brawlerData['name']) && !empty(trim($brawlerData['name'])))) {
            throw InvalidDTOException::fromMessage(
                "Invalid or missing 'name' field in battle star player brawler's data."
            );
        }
